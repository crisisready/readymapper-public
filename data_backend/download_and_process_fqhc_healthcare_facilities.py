...
FACILITIES_URL = "https://services1.arcgis.com/ZGrptGlLV2IILABw/arcgis/rest/services/FQHC_LAL_AllSites_2021/FeatureServer/0/"
FACILITIES_URL_JSON = "https://services1.arcgis.com/ZGrptGlLV2IILABw/arcgis/rest/services/FQHC_LAL_AllSites_2021/FeatureServer/0/query?where=FID+%3D+FID&outfields=*&f=json"
...
COLUMN_MAPPING = {
    ...
    'Site_Statu': 'status',
    'County_Equ': 'county',
    # 'TotalPatie': 'total_patients',
    'Latitude': 'lat',
    'Longitude': 'lon',
    'Data_Wareh': 'updated_at',
}
...
def query_arcgis_feature_server(url_feature_server=''):
    ...
    '''
    print("---> downloading data from ArcGIS feature service, this might take a few mins...")
...
def get_and_process_data(url):
    ...
    # df = pd.read_json(url)
    df = query_arcgis_feature_server(FACILITIES_URL)
...
def download_and_process_fqhc_healthcare_facilities():
    print(f"---> downaloding and processing FQHC healthcare facilities data")
    df = get_and_process_data(FACILITIES_URL)
    print("---> facility count by type")
    print(df.groupby(["type"]).size())
    print("---> setting all of them as 'Outpatient'")
    df['general_type'] = 'Outpatient'
    df.head()
    save_data(df)
